...
Изменения в файлах, которые уже были добавлены в индекс, можно не добавлять в коммит отдельными `git add`, а воспользоваться следующей командой:
...
Если добавить после этой команды число как параметр (например `git log -3`), то будет выведено только указанное количество последних коммитов.
...
Для того чтобы работать над изменениями в проекте и не сломать ничего в стабильной версии существуют ветки. По умолчанию в репозитории есть только одна ветка -- `master`. В этой ветке находится тот код, который разработчики полагают полностью работающим и готовым к использованию.
...
Подробнее о способах исправления ошибок в истории коммитов можно узнать [здесь](https://sethrobertson.github.io/GitFixUm/fixup.html).
